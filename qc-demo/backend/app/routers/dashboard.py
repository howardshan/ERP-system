...
from app.models import AppUser, Disposition, DryingSubLot, InspectionRecord, QualityEvent
from app.schemas import DashboardSummary, DispositionCreate, DispositionOut
...
def dashboard_summary(
    ...
    today_records = (
        db.query(InspectionRecord).filter(InspectionRecord.submitted_at >= today_start).all()
    )
    passed = sum(1 for r in today_records if r.result == "pass")
    failed = sum(1 for r in today_records if r.result == "fail")
    total = passed + failed
