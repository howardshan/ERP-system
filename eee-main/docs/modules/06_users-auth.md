...
**Phase 1 范围**:仅 DB + Edge Function(EF-004)+ QC 测试结果通知打通(见 09_qc.md)。
**Phase 2 待建**:① `permissionStructure.ts` 的 `auth` 模块加 `notifications` resource(谁能配置他人通知)+ seed;② UserDetail 管理员侧「通知设置」面板,按模块分卡片;③ 全新账户设置页 + TopBar 入口,供用户自助开关 `user_overridable=true` 的项。
