...
| M-172 | 20260716000001_qc_regroup_pending_sampling.sql · 新增维护 RPC `qc_regroup_pending_sampling(p_production_lot_id, p_sampling_method)`:产品取样率改动后,把某工单**尚未判定**的车按 SKU 当前 `sample_every_n_carts` 重新分组。分组在 check-out 时固化,改产品设置只影响之后出库的车,已成组的车需本 RPC 重排(再跑 check-out 无效:它要求 `status='drying'`)。只解散**全员未判定**的 `sampling` 组(含 inspecting/passed/failed/hold 成员的组整组跳过,避免 `ON DELETE SET NULL` 把已出结果的车孤立);解散前把该组仍 `pending` 的样品置 `voided`(重排后 champion 多半换车,旧实体样品不再代表该组,行保留供审计)并写 `sample_voided` 事件;重排委托 `qc__assign_test_group`(champion/车状态/`group_assigned` 事件与正常流程一致),分块循环与 M-137 check-out Step 2a 逐字一致以防 method_2 尾部均分规则漂移。未接 UI,SQL 手动调用 |
| M-173 | 20260716000002_qc_regroup_intime_window.sql · 重排 RPC 补齐 **1 小时 in_time 窗口**(BR-Q84):M-172 镜像的是 M-137 版 Step 2a(整个工单一次性切块),而 M-165 已把 check-out 改为**先按 in_time 切 1 小时窗口、再在窗口内切块**——两者分处不同分支,导致重排会把相隔数小时入烘干的车并入同一取样组,产生 check-out 永远不会产生的分组。改为 `qc__intime_windows(ids, 60)` → 逐窗口切块(`grp_seq` 跨窗口连续),与新版 Step 2a 逐字一致。其余(只解散全员未判定组 / 作废 pending 样品 / 经 `qc__assign_test_group` 重指派)不变;重排刻意按 **fresh** 处理(`redry=false`),否则 redry 的「保留原 champion」快路径会把待替换的旧分组保下来。车已出库但 `in_time` 未被清除,故窗口切分与 check-out 当时算出的完全相同。返回值新增 `windows` |
| M-174 | 20260716000003_qc_delete_production_lot.sql · 新增 `qc_delete_production_lot(p_production_lot_id)` + 权限 `production.trace.delete_work_order`(BR-Q90):删除**未开始生产**的工单——所有车仍 `created`、无 scanned_for_check_in_at、无 in_time、无取样组/样品/检测记录,否则报错拒绝。删 `qc_production_lot` 级联车(→事件/样品/spot history)+ `qc_test_group`;另删建单时预建的空 quarantine 仓库 `lot`(M-134,`wh_create_lot` 只插 lot 行无库存活动),该删除包在 EXCEPTION 里,若有下游引用则保留不中断。UI 端输入 DELETE 确认,RPC 为未开始不变量的权威校验 |
| M-175 | 20260731000001_qc_work_order_delete_audit.sql · **工单删除审计 + 加固(BR-Q91)**:事故复盘——某真实工单(16 车)无痕消失。根因:两条删除路径都零审计,其一(`LotsList` 批量删)是**前端裸删表** `supabase.from(qc_production_lot).delete()`,而该表 RLS 是 `dev_all(FOR ALL USING true)`,**任何登录用户都能直接删任何工单**,前端 `qc.batches.delete` 只是装饰。修复:①新增 **`qc_work_order_audit`** 追加式审计表(**无 FK,行随工单删除而存活**),记 actor/时间/工单号/产品/车数/车号快照/原因;②`qc_delete_production_lot(uuid,text)` 与新 `qc_delete_production_lots(uuid[],text)` 均改 **SECURITY DEFINER**、**必填原因**、删前先写审计(经内部 `qc__audit_and_delete_lot`,沿用 M-174「未开始」判定);③**收紧 `qc_production_lot` RLS**:删 `dev_all`,保留 SELECT/INSERT/UPDATE,**移除直连 DELETE**——删除只能经审计化 SECURITY DEFINER RPC(以属主身份绕过 RLS),从此不可能无痕删工单;④`v_system_audit_log` 接入 `qc_work_order_audit`,删除动作在中央日志可见。注:`qc_quality_event.drying_sub_lot_id` 是 SET NULL,故 `sub_lot_created` 事件本就不随删除消失(只是脱钩) |
| **M-176** | _(下一个)_ |
