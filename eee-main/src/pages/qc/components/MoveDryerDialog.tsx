      ...
      <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl">
        <header className="px-5 py-4 border-b border-slate-200 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-[10px] uppercase tracking-wider text-slate-500 font-bold">Move to another dryer</p>
              <h2 className="text-base font-bold text-slate-900">{count} cart(s) · Dryer {currentDryer} → ?</h2>
            </div>
          </div>
        ...
        <div className="px-5 py-4 space-y-3">
          <p className="text-xs text-slate-500">
            Choose the destination dryer. Drying time accumulates continuously — the cart stays in <code className="font-mono">drying</code> status.
          </p>
        ...
        <footer className="px-5 py-3 border-t border-slate-200 flex justify-end gap-2 bg-slate-50 rounded-b-2xl">
          ...
          >
            ...
            {busy ? 'Moving…' : `Move to Dryer ${target ?? '?'}`}
          </button>
        </footer>
      </div>
    </div>
