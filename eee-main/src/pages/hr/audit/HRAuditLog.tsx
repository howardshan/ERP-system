...
function DiffViewer({ before, after }: { before: any; after: any }) {
  ...
  if (changed.length === 0) return <p className="text-xs text-slate-400">No field changes recorded</p>;
  return (
  ...
  if (!canView) {
    return (
      <div className="min-h-screen bg-[#faf8f5] flex items-center justify-center">
        <p className="text-slate-400 text-sm">You don't have permission to view the audit log.</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#faf8f5] flex flex-col">
      <div className="px-10 pt-8 pb-5 border-b border-slate-200 bg-white">
        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">HR / Administration</p>
        <h1 className="text-2xl font-bold text-slate-900">HR Audit Log</h1>
      </div>
      ...
      <main className="flex-1 overflow-y-auto px-10 py-7">
        ...
        {loading ? (
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-10 text-center text-slate-400 text-sm">No audit log entries</div>
        ) : (
              ...
              <div key={log.id} className="border-b border-slate-100 last:border-b-0">
                <button className="w-full text-left px-5 py-3.5 hover:bg-slate-50 transition-colors"
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs text-slate-400 flex-shrink-0">{log.actor_name ?? 'System'}</span>
                ...
                {expanded === log.id && (
                  <div className="px-12 pb-4 space-y-3">
                    {log.before_snapshot || log.after_snapshot ? (
                      ...
                      <p className="text-xs text-slate-400">No snapshot data recorded for this action.</p>
                    )}
                  </div>
                )}
              </div>
