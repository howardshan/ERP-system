...
export default function SalaryRecords() {
  ...
  async function save() {
    if (!selectedEmp || !form.salary || !form.effective_date) { setErr(t('salaryRecords.errRequired')); return; }
    ...
    } catch (e: any) { setErr(e?.message ?? t('salaryRecords.error')); }
    setSaving(false);
  }
  ...
  return (
    <div className="min-h-screen bg-[#faf8f5] flex flex-col">
      <div className="px-10 pt-8 pb-5 border-b border-slate-200 bg-white">
        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">{t('salaryRecords.breadcrumb')}</p>
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold text-slate-900">{t('salaryRecords.title')}</h1>
          {canManage && selectedEmp && (
            <button onClick={() => { setModal(true); setErr(''); }} className="flex items-center gap-1.5 px-4 py-2 bg-teal-600 hover:bg-teal-500 text-white text-xs font-bold rounded-lg transition-colors">
              <Plus size={14} /> {t('salaryRecords.addRecord')}
            </button>
          )}
        </div>
      </div>
      ...
      <main className="flex-1 overflow-y-auto px-10 py-7">
        <div className="mb-6 max-w-xs">
          <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">{t('salaryRecords.selectEmployee')}</label>
          <select value={selectedEmp} onChange={e => setSelectedEmp(e.target.value)}
            ...
            <option value="">{t('salaryRecords.selectEmployeePlaceholder')}</option>
        ...
        {!selectedEmp ? (
          <div className="flex flex-col items-center justify-center py-20 gap-3 text-slate-400">
            ...
            <p className="text-sm">{t('salaryRecords.emptySelect')}</p>
          </div>
        ) : loading ? (
          ...
          <div className="space-y-5">
            {currentSalary && (
              <div className="bg-white border border-slate-200 rounded-xl p-6">
                <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">{t('salaryRecords.currentSalary')}</h3>
                ...
                <p className="text-xs text-slate-400 mt-1">{t('salaryRecords.effective')}: {currentSalary.effective_date}{currentSalary.reason ? ` · ${currentSalary.reason}` : ''}</p>
              </div>
            )}

            <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
              <div className="px-6 py-4 border-b border-slate-200">
                <h3 className="text-sm font-bold text-slate-900">{t('salaryRecords.history')}</h3>
              </div>
              {history.length === 0 ? (
      ...
      {modal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            <h2 className="text-lg font-bold text-slate-900 mb-5">{t('salaryRecords.addRecord')}</h2>
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">{t('salaryRecords.effectiveDateRequired')}</label>
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">{t('salaryRecords.salaryRequired')}</label>
                  <input type="number" min={0} value={form.salary} onChange={e => setForm(p => ({ ...p, salary: e.target.value }))} placeholder={t('salaryRecords.salaryPlaceholder')}
                    className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500" />
                </div>
              </div>
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">{t('salaryRecords.frequency')}</label>
                  <select value={form.pay_frequency} onChange={e => setForm(p => ({ ...p, pay_frequency: e.target.value }))}
                    className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500">
                    <option value="monthly">{t('salaryRecords.freqMonthly')}</option>
                    <option value="bi_weekly">{t('salaryRecords.freqBiWeekly')}</option>
                    <option value="weekly">{t('salaryRecords.freqWeekly')}</option>
                  </select>
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">{t('salaryRecords.currency')}</label>
                  <input value={form.currency} onChange={e => setForm(p => ({ ...p, currency: e.target.value }))}
                    className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500" />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">{t('salaryRecords.payGrade')}</label>
                  <input value={form.pay_grade} onChange={e => setForm(p => ({ ...p, pay_grade: e.target.value }))} placeholder={t('salaryRecords.payGradePlaceholder')}
                    className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500" />
                </div>
              </div>
              <div>
                <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">{t('salaryRecords.reason')}</label>
                <input value={form.reason} onChange={e => setForm(p => ({ ...p, reason: e.target.value }))} placeholder={t('salaryRecords.reasonPlaceholder')}
                  className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500" />
              </div>
            </div>
            {err && <p className="mt-3 text-xs text-red-500">{err}</p>}
            <div className="flex justify-end gap-3 mt-6">
              <button onClick={() => setModal(false)} className="px-4 py-2 text-sm text-slate-600 hover:bg-slate-100 rounded-lg">{t('salaryRecords.cancel')}</button>
              <button onClick={save} disabled={saving}
                className="px-4 py-2 bg-teal-600 hover:bg-teal-500 disabled:opacity-50 text-white text-sm font-bold rounded-lg">
                {saving ? t('salaryRecords.saving') : t('salaryRecords.save')}
              </button>
            </div>
          </div>
        </div>
