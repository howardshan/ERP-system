...
export default function JobRequisitions({ onSelectRequisition }: Props) {
  ...
  async function save() {
    if (!modal.req?.title) { setErr('Title is required'); return; }
            ...
            {filtered.length === 0 ? (
              ...
              <table className="w-full">
                <thead><tr className="bg-slate-50 border-b border-slate-200">
                  {['Title','Department','Hiring Manager','Status','Headcount','Target Date',''].map(h =>
                  ...
                  {filtered.map(r => (
                    <tr key={r.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-5 py-3.5 flex gap-2">
                        <button onClick={() => onSelectRequisition(r.id)} className="text-xs text-teal-600 hover:underline font-semibold">Candidates</button>
                        {canEdit && <button onClick={() => openEdit(r)} className="text-xs text-slate-400 hover:text-slate-700">Edit</button>}
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      {modal.open && modal.req && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
            <h2 className="text-lg font-bold text-slate-900 mb-5">{modal.req.id ? 'Edit' : 'New'} Job Requisition</h2>
            <div className="space-y-4">
              <div>
                <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Job Title *</label>
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Hiring Manager</label>
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Salary Min</label>
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Salary Max</label>
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Target Fill Date</label>
                <input type="date" value={modal.req.target_fill_date ?? ''} onChange={e => setModal(p => ({ ...p, req: { ...p.req!, target_fill_date: e.target.value || null } }))}
                  className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500" />
              </div>
              <div>
                <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Job Description</label>
            ...
            <div className="flex justify-end gap-3 mt-6">
              <button onClick={() => setModal({ open: false, req: null })} className="px-4 py-2 text-sm text-slate-600 hover:bg-slate-100 rounded-lg">Cancel</button>
