...
app.post('/print', async (req, res) => {
  ...
  if (body.pdf) {
    ...
    try {
      ...
      if (IS_WINDOWS) {
        // SumatraPDF fallback when client still sends { pdf }. Prefer { pngs } from
        // the web app on Windows — PrintDocument with fixed PaperSize 4×3.
        // `noscale` respects PDF page geometry (no auto-rotate like `fit`).
