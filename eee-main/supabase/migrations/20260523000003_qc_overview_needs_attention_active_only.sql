-- M-064: needs_attention should only show items that still require action.
--
-- Root cause: qc_overview's needs_attention query returned every fail
-- inspection from the last 24 h with no regard for the carts' current
-- status.  Carts that were already disposed (now 'drying', 'awaiting_recheck',
-- 'closed', etc.) kept appearing with the DRYING label because the query
-- only filters on ir.submitted_at, not on whether action is still pending.
--
-- Fix: add two conditions to the WHERE clause:
--   1. ir.result = 'fail'  (pass results never need attention)
--   2. At least one cart in the group is still in 'hold' or
--      'awaiting_group_result' (= the group has NOT yet been fully acted on).
--      For solo carts the champion itself must be in one of those statuses.
        ...
        'needs_attention', COALESCE((
            ...
            FROM (
                SELECT jsonb_build_object(
                    ...
                    -- Sampling group info (null fields when cart has no group)
                ...
                WHERE ir.submitted_at >= now() - interval '24 hours'
                  AND ir.result = 'fail'
                  -- Only show if there is still at least one cart in the group
                  -- that has NOT yet been acted on (still in hold / awaiting_group_result).
                  AND (
                    CASE
                      WHEN s.test_group_id IS NOT NULL THEN
                        EXISTS (
                      ...
                      ELSE
                        s.status IN ('hold', 'awaiting_group_result')
                    END
                  )
