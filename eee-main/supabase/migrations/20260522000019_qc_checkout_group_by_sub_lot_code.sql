-- M-061: Fix sampling-group assignment order.
--
-- Root cause: qc_check_out_sub_lots_bulk grouped carts using
--   array_agg(s.id ORDER BY s.created_at)
-- All carts in the same production batch are created within milliseconds of
-- each other, so the ORDER BY produced an arbitrary sequence.  This caused
-- non-consecutive carts (e.g. 001, 004, 006) to end up in the same group
-- instead of the expected sequential chunks (001-002-003, 004-005-006, …).
--
-- Fix: order by sub_lot_code instead.  The code is formatted as
-- {work_order_barcode}-{NNN} (zero-padded sequence), so lexicographic order
-- gives the correct 001 → 002 → 003 → … sequence.
...
BEGIN
    ...
    -- Order by sub_lot_code (lexicographic = sequential for zero-padded codes like -001, -002 …)
    -- so that consecutive carts end up in the same group (001-002-003, 004-005-006, …).
    FOR grp_rec IN
