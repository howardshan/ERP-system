-- ─────────────────────────────────────────────────────────────────────────────
-- M-138  Multi-test inspection: submit ALL test readings at once
--
-- A SKU can have several inspection templates (e.g. Water Activity + MC%). The
-- old qc_submit_inspection took a single p_aw and only the first template, so a
-- champion was judged on one measurement. Now the operator enters a reading for
-- every configured test and the champion is judged on all of them together.
--
-- New optional param p_values jsonb = { "<template_id>": <numeric>, ... } keyed
-- by qc_inspection_template.id. When present (multi-test mode):
--   • every template for the SKU MUST have a reading (else EXCEPTION);
--   • each reading is banded against its own hard/soft limits;
--   • overall in_hard = ALL readings in hard range; overall in_soft = ALL in soft;
--   • suggested = pass iff all in hard; the supervisor-override gate uses the
--     overall in_soft exactly as before.
-- When p_values is NULL the legacy single-p_aw path runs unchanged (bulk submit).
--
-- values_json now stores { readings:{id:{...}}, suggested, in_hard, in_soft, aw }
-- in multi mode (aw kept = the Water-Activity reading for report back-compat),
-- and the legacy { aw, ... } shape in single mode.
-- ─────────────────────────────────────────────────────────────────────────────
