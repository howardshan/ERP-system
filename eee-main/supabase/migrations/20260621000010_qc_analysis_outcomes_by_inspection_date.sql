-- ─────────────────────────────────────────────────────────────────────────────
-- M-143  Outcomes daily / by-work-order keyed by FIRST-INSPECTION date
--
-- The metrics tile (M-142) now counts first-test pass/fail by activity in range,
-- but the "Daily trend" chart (qc_analysis_outcomes_daily) and its per-work-order
-- breakdown (qc_analysis_outcomes_by_work_order) still scoped + dated everything
-- by out_time. A cart that failed and was sent back to re-dry has out_time = NULL,
-- so it never appeared on the chart / breakdown — fail showed 0 there even though
-- the tile showed 2.
--
-- FIX: both now scope by sku/lot/dryer only (no out_time gate) and date each cart
-- by its FIRST inspection's submitted_at. Range / day filters apply to that
-- inspection date. This matches the tile and the operator's model: a first-test
-- result counts on the day it was tested, regardless of dryer state.
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
    WITH scope AS (
        ...
        WHERE (p_sku_id IS NULL OR pl.sku_id = p_sku_id)
          ...
          AND (
            ...
            OR EXISTS (SELECT 1 FROM qc_sub_lot_spot_history h
    ...
    sibling_insp AS (
        ...
        JOIN qc_drying_sub_lot champ
          ON champ.test_group_id = sc.test_group_id AND champ.is_test_champion = true
        JOIN direct_insp di ON di.sub_lot_id = champ.id
        WHERE sc.test_group_id IS NOT NULL AND sc.is_test_champion = false
          AND NOT EXISTS (SELECT 1 FROM qc_inspection_record ir2 WHERE ir2.drying_sub_lot_id = sc.sub_lot_id)
    ),
    first_insp AS (
