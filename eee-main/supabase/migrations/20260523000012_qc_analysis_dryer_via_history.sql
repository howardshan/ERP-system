-- Migration: Analysis dryer filter — match via spot history, not just current
-- dryer_number.
--
-- Bug: once a cart is bulk-checked-out, its qc_drying_sub_lot.dryer_number is
-- set to NULL (so the cart isn't double-counted as "occupying" a dryer slot).
-- The Analysis page's dryer filter used `COALESCE(s.dryer_number, l.dryer_number)`
-- which is NULL for any post-checkout cart, so filtering by a specific dryer
-- excluded all passed/closed/inspected carts that had been in that dryer.
--
-- Fix: when p_dryer_number is supplied, a cart matches if EITHER its current
-- dryer is that number OR qc_sub_lot_spot_history has any row for that
-- (drying_sub_lot_id, dryer_number). History is the source of truth for
-- "this cart was in Dryer N at some point".
