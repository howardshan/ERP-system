-- Migration M-056: Enrich qc_overview needs_attention rows with sampling-group
-- member info so the UI can show one row per sample (= one row per group test)
-- instead of one row per cart.
--
-- New fields added to each needs_attention item:
--   test_group_id        — uuid or null (null = solo cart, no group)
--   group_sub_lot_ids    — jsonb array of ALL sub-lot UUIDs in the group
--   group_sub_lot_codes  — jsonb array of ALL sub-lot codes in the group
--   group_size           — total cart count in the group (1 for solo carts)
