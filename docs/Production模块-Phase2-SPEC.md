...
> **状态**: v0.3 —— M1 决策(D1–D10)已拍板。**M1.1(工单主数据 + `prod_run` 单一事实源 + 工单驱动录入)已落地(M-124)**;下一步 M1.2 平板端(打卡/停机)。
> **作者**: —
> **日期**: 2026-06-11
...
| 里程碑 | 交付 | 主要新建 |
|--------|------|----------|
| **M1.1** 工单 + 录入地基 ✅ **已落地(M-124)** | F1 工单主数据;F3 工单驱动录入;方案 A 收敛 `prod_run` | `prod_work_order` + 工单页 + `prod_run`/`prod_run_view` + rollup 视图 |
| **M1.2** 平板端 | F2 平板登录/打卡;F3 平板录入;F4 停机实时 | `prod_line_attendance`、`prod_line_device`、`prod_downtime_event`、平板 UI |
