...
> **状态**: v0.5 —— M1 决策(D1–D10)已拍板。**M1.1 已落地(M-125)**;**M1.2a 平板 kiosk + 打卡已落地(M-126)**;**M1.2b 平板生产录入 + 跨班续做车 + 停机实时已落地(M-127)**;下一步 M1.3 工时汇总 + 实时看板。
> **作者**: —
> **日期**: 2026-06-11
...
| **M1.2b** 平板录入 + 停机 ✅ **已落地(M-127)** | F3 平板生产录入(source=tablet)+ 续做车(D8);F4 停机实时 | `prod_downtime_event`、`submitTabletRun`、平板 Tab 工作台(生产/停机) |
